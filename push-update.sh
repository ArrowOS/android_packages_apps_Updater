...
updates_dir=/data/arrowos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have Arrow-VERSION-DATE-TYPE-* as format"
...
# Assume Arrow-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.arrowos.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.arrowos.updater/databases/updates.db" \
