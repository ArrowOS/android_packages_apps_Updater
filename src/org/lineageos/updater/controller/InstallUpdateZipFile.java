    ...
    public void prepareZip() {
        ...
        if (zipFileName.contains(device)) {
            Thread thread = new Thread(() -> {
                boolean hasReadPermission = PermissionsUtils.checkAndRequestStoragePermission((UpdatesListActivity) mActivity, 0);
                try {
                    if (!hasReadPermission) throw new IOException();
                ...
                mActivity.runOnUiThread(() -> {
                    if (progressDialog != null && hasReadPermission) {
                        progressDialog.hide();
