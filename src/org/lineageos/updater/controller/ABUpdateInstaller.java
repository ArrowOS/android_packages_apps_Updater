...
class ABUpdateInstaller {
    ...
    private final UpdateEngineCallback mUpdateEngineCallback = new UpdateEngineCallback() {
        ...
        public void onPayloadApplicationComplete(int errorCode) {
            if (errorCode != UpdateEngine.ErrorCodeConstants.SUCCESS && !mDownloadId.isEmpty()) {
                installationDone(false);
                Update update = mUpdaterController.getActualUpdate(mDownloadId);
                update.setInstallProgress(0);
    ...
    public boolean install(File file) {
        ...
        // Disable performance mode if it isn't from local downloads DB
        if (mDownloadId.isEmpty()) mUpdateEngine.setPerformanceMode(false);
